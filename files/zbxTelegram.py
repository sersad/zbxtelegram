...
# Патчим внутренний метод telebot для использования нашей сессии
...
# Теперь создаём бота обычным способом
bot = telebot.TeleBot(args.token if args.token else tg_token)
if tg_proxy:
    apihelper.proxy = tg_proxy_server
...
# ВСТАВИТЬ ЭТУ ФУНКЦИЮ ВНУТРЬ main(), ПОСЛЕ СТРОКИ: data_zabbix = xml_parsing(args.messages)
def gen_markup(eventid, itemid=None):
    """Создаёт клавиатуру с компактными колбэками (≤64 байт)"""
    markup = InlineKeyboardMarkup()
    markup.row_width = zabbix_keyboard_row_width
    
    # Формат: <действие>:<eventid>[:<itemid>]
    # "a" = acknowledge, "m" = messages, "h" = history, "l" = last value, "g" = graph
    buttons = [
        InlineKeyboardButton("💬 messages", callback_data=f"m:{eventid}"),
        InlineKeyboardButton("✅ acknowledge", callback_data=f"a:{eventid}"),
        InlineKeyboardButton("📈 history", callback_data=f"h:{eventid}"),
        InlineKeyboardButton("⏱ last value", callback_data=f"l:{eventid}"),
    ]
    
    # Добавляем кнопку графика если есть валидный itemid
    if itemid and re.search(r'\d+', itemid):
        clean_itemid = re.search(r'\d+', itemid).group()
        buttons.append(InlineKeyboardButton("📊 graph", callback_data=f"g:{eventid}:{clean_itemid}"))
    
    markup.add(*buttons)
    return markup


def send_messages(sent_to, message, graphs_png, eventid=None, itemid=None, settings_keyboard=None, disable_notification=False):
    try:
...
def main():
    ...
    if args.subject in ['Test subject', 'test', 'Тестовая тема'] or args.messages in \
        ...
        send_messages(
            ...
            eventid="12345",          # Валидный фейковый eventid для колбэков
            itemid="67890",           # Валидный фейковый itemid для кнопки графика
            settings_keyboard=True,   # <-- КЛЮЧЕВОЕ ИЗМЕНЕНИЕ: включаем кнопки
    ...
    #     message, 
    #     graphs_png, 
    #     data_zabbix['eventid'], data_zabbix.get('settings_keyboard_bool'),
    #     disable_notification=True if isinstance(zntsettings_tags, dict) and trigger_settings_tag_not_notify in zntsettings_tags[trigger_settings_tag] else False
    #     )
    send_messages(
        sent_to=args.username,
        message=message,
        ...
        itemid=data_zabbix.get('itemid'),  # <-- ДОБАВЛЕНО
